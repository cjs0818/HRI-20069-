...
```
  ...
  2. The scripts require root or sudo privileges to run. So, for convenience, you'd better add $USER to the docker group and reboot.
```
...
```
  ...
  4. Sign up for Docker (https://cloud.docker.com/) and login.
```
    $ docker login
```
